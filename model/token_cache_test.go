...
type failNextTokenCacheCommandHook struct {
	fail atomic.Bool
}

func (h *failNextTokenCacheCommandHook) BeforeProcess(ctx context.Context, _ redis.Cmder) (context.Context, error) {
	if h.fail.CompareAndSwap(true, false) {
		return ctx, errors.New("forced token cache command failure")
	}
	return ctx, nil
}

func (h *failNextTokenCacheCommandHook) AfterProcess(context.Context, redis.Cmder) error {
	return nil
}

func (h *failNextTokenCacheCommandHook) BeforeProcessPipeline(ctx context.Context, _ []redis.Cmder) (context.Context, error) {
	return ctx, nil
}

func (h *failNextTokenCacheCommandHook) AfterProcessPipeline(context.Context, []redis.Cmder) error {
	return nil
}
...
func TestTokenUpdateInvalidatesCacheBeforeReturning(t *testing.T) {
	useTokenCacheMiniRedis(t)
	oldDB := DB
	...
	if _, err := cacheGetTokenByKey(token.Key); err == nil {
		t.Fatal("expected token update to invalidate the cached snapshot")
	}
}
...
func TestTokenMaxGroupRatioTighteningStopsBeforeDatabaseWhenCacheFails(t *testing.T) {
	...
	hook := &failNextTokenCacheCommandHook{}
